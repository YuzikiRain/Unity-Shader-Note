...
从观察空间变换到裁剪空间的矩阵被称为裁剪矩阵或投影矩阵

裁剪空间中，

但实际的投影（从三维到二维）发生在NDC坐标空间上，（然后经过透视除法和屏幕映射（视口变换）变换到屏幕空间上）

### 观察空间到裁剪空间：裁剪（投影）变换

投影是为了将三维空间中的点映射到二维屏幕空间上，其实就是映射到近平面上

![q1SNB](https://fastly.jsdelivr.net/gh/YuzikiRain/ImageBed/img/202208301217034.png)
...
![image-20220830102526211](https://fastly.jsdelivr.net/gh/YuzikiRain/ImageBed/img/202208301025196.png)
...
\left[\begin{matrix}x\\y\\z\\1\end{matrix}\right]
$$
![image-20220830120610406](https://fastly.jsdelivr.net/gh/YuzikiRain/ImageBed/img/202208301206481.png)
...
![image-20220830120436904](https://fastly.jsdelivr.net/gh/YuzikiRain/ImageBed/img/202208301204986.png)
...
\right]
$$
注意第四行为$[0,0,1,0]$，观察空间下坐标$(x',y',z',z)$经过投影变换到裁剪空间下为$(x',y',z',z)$，齐次除法（各分量同时除以$w$分量）后变换到NDC，$w$分量变为1，此时$xyz$分量范围为$[-1,1]$。反推得到齐次除法之前（即裁剪空间下）$xyz$分量范围为$[-w,w]$
